...
use data\model\NsGoodsStockModel;
...
class Order extends BaseController
{
    ...
    public function paymentOrder()
    {
        ...
        if (($order_tag == "buy_now" && $order_goods_type == "0") 
            || ($order_tag == "cart" && $order_goods_type == "0")) {
            // 虚拟商品
    ...
    public function addShoppingCartSession()
    {
        ...
        for ($i = 0; $i < count($cart_list); $i ++) {
            if ($cart_id_arr[$i] == $cart_list[$i]["cart_id"]) {
                ...
                $goods_sku_list .= "," . $cart_list[$i]['sku_id'] . ':' . $cart_list[$i]['num'] . ':' . $cart_list[$i]['appointment_date'];
            }
        }
    ...
    public function buyNowSession()
    {
        ...
        $appointment_date = $order_sku_list[2];
        ...
        if (! empty($appointment_date)) {
            ...
            $goods_stock_info = $goods_stock_model->getInfo([
                'goods_id' => $sku_info["goods_id"],
                'sku_id' => $sku_id,
                'appointment_date' => $appointment_date
            ], '*');
            $sku_info['price'] = $goods_stock_info['price'];
            $sku_info['promote_price'] = $goods_stock_info['promote_price'];
            $sku_info['stock'] = $goods_stock_info['stock'];
        }
        ...
        $cart_list["appointment_date"] = $appointment_date;
        
        $goods_preference = new GoodsPreference();
        $member_price = $goods_preference->getGoodsSkuMemberPrice($sku_info['sku_id'], $this->uid, $appointment_date);
        ...
        $list[] = $cart_list;
        $goods_sku_list = $sku_id . ":" . $num . ":" . $appointment_date; // 商品skuid集合
        $res["list"] = $list;
        $res["goods_sku_list"] = $goods_sku_list;
        return $res;
    }
    ...
    public function orderCreateSession()
    {
        ...
        if ($tag == 'cart') {
            ...
            // 加入购物车
            $cart_id_list = request()->post('cart_id');
            $_SESSION['cart_list'] = $cart_id_list;
            $cart_model = new NsCartModel();
            $cart_info_list = $cart_model->getQuery([
                'cart_id' => [
                    'in',
                    $cart_id_list
                ]
            ], "*", "");
            $exist_real_good = false;
            $exist_virt_good = false;
            foreach ($cart_info_list as $cart_info) {
                if ($cart_info['goods_type'] == 1) {
                    $exist_real_good = true;
                } else {
                    $exist_virt_good = true;
                }
            }
            if ($exist_real_good && $exist_virt_good) {
                return -1;
            }
            if ($exist_real_good) {
                $_SESSION['order_goods_type'] = "1"; // 商品类型标识
            } else {
                $_SESSION['order_goods_type'] = "0"; // 商品类型标识
            }
        }
        if ($tag == 'buy_now') {
            $_SESSION['order_tag'] = 'buy_now';
            $_SESSION['order_sku_list'] = request()->post('sku_id') . ':' . request()->post('num') . ':' . request()->post('appointment_date');
    ...
    public function reviewCommodity()
    {
        ...
        if (request()->isGet()) {
            ...
            if (($orderDetail['order_status'] == 1 || $orderDetail['order_status'] == 4) && $orderDetail['is_evaluate'] == 0) {} else {
    ...
    public function reviewAgain()
    {
        ...
        if (request()->isGet()) {
            ...
            $this->assign("order_id", $order_id);
            $this->assign("list", $list);
            if (($orderDetail['order_status'] == 1 || $orderDetail['order_status'] == 8) && $orderDetail['is_evaluate'] == 1) {

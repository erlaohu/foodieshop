...
class Goods extends BaseController
{
    ...
    public function addCart()
    {
        ...
        if (! empty($this->uid) && $this->is_member == 1) {
            $goods = new GoodsService();
            $retval = $goods->addCart($uid, $shop_id, $shop_name, $goods_id, $goods_name, $goods_type, $sku_id, $sku_name, $appointment_date, $price, $num, $picture, 0);
        } else {
            $retval = array(
                "code" => - 1,
                "message" => ""
    ...
    public function areaZone()
    {
        ...
        if (request()->isAjax()) {
            ...
            $goods_list = $this->goods->getGoodsList($page_index, PAGESIZE, $condition, "", $city_id);
            return $goods_list;
        } else {
            // 地区列表
    ...
    public function bargainList(){
        ...
        if(request()->isAjax()){
            ...
            $bargain = new Bargain();
            $list = $bargain->getBargainGoodsPage($page_index, $page_size, $condition);
            return $list;
        }

...
class Goods extends BaseController
{
    ...
    public function addCart()
    {
        ...
        $goods_type = $cart_detail['goods_type'];
        $shop_id = $this->instance_id;
        $web_info = $this->web_site->getWebSiteInfo();
        $count = $cart_detail['count'];
        $sku_id = $cart_detail['sku_id'];
        $sku_name = $cart_detail['sku_name'];
        $appointment_date = $cart_detail['appointment_date'];
        $price = $cart_detail['price'];
        $cost_price = $cart_detail['cost_price'];
        $picture_id = $cart_detail['picture_id'];
        $_SESSION['order_tag'] = ""; // 清空订单
        $retval = $goods->addCart($uid, $shop_id, $web_info['title'], $goods_id, $goods_name, $goods_type, $sku_id, $sku_name, $appointment_date, $price, $count, $picture_id, 0);
        return $retval;
    }
    ...
    public function areaZone()
    {
        ...
        $groupList = $dataAddress->getCityList(999);
        $this->assign("groupList", $groupList);

        if (! empty($groupList["data"]) && empty($city_id)) {
            $city_id = $groupList["data"][0]["city_id"];
        }
        // 标签列表
        $this->assign("groupList", $groupList);
        $this->assign("city_id", $city_id);
        
        // $curr_group = $goods_group->getGoodsGroupDetail($city_id);
        $this->assign("curr_group", array());
        
        $this->goods = new GoodsService();
        $condition = "";
        
        if (! empty($city_id)) {
            $condition = "FIND_IN_SET(" . $city_id . ",ng.area_id_array)";
        }
        $goods_list = $this->goods->getGoodsList($page_index, PAGESIZE, $condition, "");
        $this->assign("goods_list", $goods_list);
        $this->assign('page_count', $goods_list['page_count']);
        ...
        $this->assign("title_before", "地区美食");
        return view($this->style . 'Goods/areaZone');
    }
    ...
    public function getGoodsStockList()
    {
        ...
        $goods = new GoodsService();
        return $goods->getGoodsStockList($condition);
    }

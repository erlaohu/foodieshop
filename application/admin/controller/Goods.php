...
class Goods extends BaseController
{
    ...
    public function addGoods()
    {
        ...
        $cityList = $dataAddress->getCityList(999);
        $this->assign("area_list", $cityList);
...
$product['goods_attribute_id'], $product['goods_attribute'], $product['goods_spec_format'], $product['goods_weight'], $product['goods_volume'], $product['shipping_fee_type'], $product['categoryExtendId'], $product["sku_picture_vlaues"], $product['virtual_goods_type_data'], $product['production_date'], $product['shelf_life'], $product['ladder_preference'], $product['goods_video_address'], $product['pc_custom_template'], $product['wap_custom_template'], $product['max_use_point'], $product['is_open_presell'], $product['presell_delivery_type'], $product['presell_price'], $product['presell_time'], $product['presell_day'], $product['goods_unit'], $product["appointment_day"], $product["areaArray"]);
     ...
     * 批量設置
     */
    public function batchSettingStock()
    {
        if (request()->isAjax()) {
            $info = array(
                "goods_id" => request()->post("goods_id", 0),
            ...
            $goods = new GoodsService();
            $res = $goods->batchSettingStock($info);
            return $res;
        }
    }

...
$(function() {
	...
	$('.appointment-date-input').focus(function() {
		var appointment_date = $("#hiddAppointmentDate").val();
		var appointment_date_ary = appointment_date.split("-");
		$(this).date({
			curdate:true,
			beginyear: parseInt(appointment_date_ary[0], 10),
			beginmonth: parseInt(appointment_date_ary[1], 10),
			beginday:parseInt(appointment_date_ary[2], 10),
			endyear:2050,
			// theme:'datetime'
		},function(){
			var goods_id = $("#goods_id").val();
			$.ajax({
				url : __URL(SHOPMAIN+"/goods/getGoodsStockList"),
				type : "post",
				async: false,
				data : {
					"goods_id" : goods_id,
				...
				success : function(res){
					if (res.length > 0) {
						for (var i = 0;i < res.length; i++) {
							...
							var attrSkuId = "[skuid=" + responseData["sku_id"] + "]";
							var attrSkuId = "[skuid=" + responseData["sku_id"] + "]";
							$(attrSkuId).attr("stock", responseData["stock"]);
							$(attrSkuId).attr("price", responseData["price"]);
							$(attrSkuId).attr("promotion_price", responseData["promote_price"]);
						}
					} else {
	...
	// $('.appointment-date-input').datetimepicker({
	// 	minDate: 0,
	// 	timepicker:false,
	// 	format:'Y-m-d',
	...
	$('#submit_ok,.address_block').bind("click",function() {
		if($("#uid").val() == null || $("#uid").val() == ""){
			...
			if ($("#hiddStock").val() == 0) {
				...
				var flag = 0;
				// 预约日期
				if ($("#hidden_goods_type").val() == "0") {
					if ($("[name='appointment_date']").val() == "") {
						showBox("请选择预约日期","warning");
						return;
					} else {
						flag++;
					}
					...
					if (num >= 1) {
						if (num <= nummax) {
							if (num <= purchaseSum|| purchaseSum == 0) {
								...
								cart_detail.goods_type = $("#hidden_goods_type").val();
								cart_detail.appointment_date = $("[name='appointment_date']").val();
								//没有SKU商品，获取第一个
									...
									getGoodsPurchaseRestrictionForCurrentUser($("#goods_id").val(),num,function(purchase){
										if(purchase.code>0){
											$.ajax({
												...
												type : "post",
												data : { "tag" : "buy_now", "sku_id" : skuid, "num" :num, "appointment_date" : $("[name='appointment_date']").val(), "goods_type" : $("#hidden_goods_type").val() },
												success : function(res){
...
function echoSpecData(){
	...
	$(".sku-array").each(function(i) {
		...
		if(JSON.stringify(sku_array) == JSON.stringify(spec_array)){
			...
			var sku_appointment_day = parseInt($this.attr("skuday"));
			if (sku_appointment_day > 0) {
				$(".appointment-day-span").text("需要提前 " + sku_appointment_day + " 天预约");
			} else {
				$(".appointment-day-span").text("");
			}
			var appointment_date = new Date(Date.parse(new Date()) + 86400000 * sku_appointment_day);
			var appointment_date_min = formatDate(appointment_date, 'yyyy-MM-dd');
			if ($(".appointment-date-input").val() != "" && $(".appointment-date-input").val() < appointment_date_min) {
				$(".appointment-date-input").val(appointment_date_min);
			}
			$("#hiddAppointmentDate").val(appointment_date_min);
			// $(".appointment-date-input").datetimepicker({
			// 	minDate : appointment_date,
			// 	defaultDate : appointment_date
			// });
			//当前已选
			if(select_skuName != null){
				var selected = $("[name='appointment_date']").val() == undefined ? '' : $("[name='appointment_date']").val() + '&nbsp;' + select_skuName;
				if($("#num").val() > 0){

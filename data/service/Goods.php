...
use data\model\NsGoodsStockModel as NsGoodsStockModel;
...
class Goods extends BaseService implements IGoods
{
        ...
        try {
            ...
            if ($goods_type == 0) {
                ...
                if ($virtual_data['value_info'] == '') {
                    ...
                    $value_info = $virtual_data['value_info'];
                }
                
                $virtual_goods_service = new VirtualGoods();
                $res_virtual_goods_type = $virtual_goods_service->editVirtualGoodsType(0, $virtual_data['virtual_goods_group_id'], $virtual_data['validity_period'], $virtual_data['confine_use_number'], $value_info, $goods_id);
            }
    ...
    public function getBusinessGoodsInfo($goods_detail)
    {
        ...
        $goods_preference = new GoodsPreference();
        // 产品库存表
        $goods_stock_model = new NsGoodsStockModel();
        ...
        foreach ($goods_detail['sku_list'] as $k => $goods_sku) {
            // 虚拟商品的时候
            if ($goods_detail['goods_type'] == 0) {
                // 预约日期
                $goods_detail['appointment_date'] = Time::dateAfter($goods_sku['appointment_day']);
                $goods_stock_info = $goods_stock_model->get([
                    "goods_id" => $goods_detail['goods_id'],
                    "sku_id" => $goods_detail['sku_list'][$k]['sku_id'],
                    "appointment_date" => $goods_detail['appointment_date']
                ]);
                if (! empty($goods_stock_info)) {
                    $goods_detail['sku_list'][$k]['price'] = $goods_stock_info['price'];
                    $goods_detail['sku_list'][$k]['promote_price'] = $goods_stock_info['promote_price'];
                    $goods_detail['sku_list'][$k]['stock'] = $goods_stock_info['stock'];
                } else {
                    $goods_detail['sku_list'][$k]['stock'] = 0;
                }
            }
        ...
        if (! empty($cart_goods_list)) {
            foreach ($cart_goods_list as $k => $v) {
                ...
                if (! empty($v['appointment_date'])) {
                    $goods_stock_model = new NsGoodsStockModel();
                    $goods_stock = $goods_stock_model->getInfo([
                        'goods_id' => $v['goods_id'],
                        'sku_id' => $v['sku_id'],
                        'appointment_date' => $v['appointment_date'],
                    ], 'stock, price, promote_price');
                    $sku_info["price"] = $goods_stock["price"];
                    $sku_info["promote_price"] = $goods_stock["promote_price"];
                    $sku_info["stock"] = $goods_stock["stock"];
                }
                // 将goods_id 存放到数组中
     ...
     */
    public function addCart($uid, $shop_id, $shop_name, $goods_id, $goods_name, $goods_type, $sku_id, $sku_name, $appointment_date, $price, $num, $picture, $bl_id)
    {
        $retval = array(
            'code' => 0,
            "message" => ""
    ...
    public function copyGoodsInfo($goods_id)
    {
        ...
        foreach ($goods_detail['sku_list'] as $item) {
            if (! empty($item['attr_value_items'])) {
                $skuArray .= $item['attr_value_items'] . '¦' . $item['price'] . "¦" . $item['market_price'] . "¦" . $item['cost_price'] . "¦" . $item['stock'] . "¦" . $item['appointment_day'] . "¦" . $item['code'] . '§';
            }
        }
    ...
    /*
     * (non-PHPdoc)
     * @see \data\api\IGoods::getGoodsStockList()
     */
    public function getGoodsStockList($condition)
    {
        // TODO Auto-generated method stub
        $goods_stock_model = new NsGoodsStockModel();
        $query = $goods_stock_model->getQuery($condition, "*", "");
        return $query;
    }
     ...
     * @param unknown $info            
     */
    public function batchSettingStock($info)
    {
        $date_array = explode(',', $info['dates']);
        $goods_model = new NsGoodsModel();
        $goods_sku_model = new NsGoodsSkuModel();
        $goods_stock_model = new NsGoodsStockModel(); // 产品库存表
                                                      // 开启事物
        $goods_stock_model->startTrans();
        ...
        try {
            ...
            if ($info['price'] != 0) {
                foreach ($date_array as $v) {
                    $goods_data = array(); // 商品修改项
                    $goods_data['goods_id'] = $info['goods_id'];
                    $goods_data['sku_id'] = $info['sku_id'];
                    $goods_data['appointment_date'] = $v;
                    $goods_data['price'] = $info['price'];
                    $goods_data['promote_price'] = $info['price'];
                    $goods_data['stock'] = $info['stock'];
                    $goods_stock_model = new NsGoodsStockModel(); // 产品库存表
                    $find_id = $goods_stock_model->get([
                        "goods_id" => $info['goods_id'],
                        "sku_id" => $info['sku_id'],
                        "appointment_date" => $v
                    ]);
                    if (! empty($find_id)) {
                        $goods_stock_model->save($goods_data, [
                            "goods_id" => $info['goods_id'],
                            "sku_id" => $info['sku_id'],
                            "appointment_date" => $v
                        ]);
                ...
                $goods_stock_model->commit();
            } else {
                $date_array = implode($date_array, ",");
                $res = $goods_stock_model->destroy([
                    "goods_id" => $info['goods_id'],
                    "sku_id" => $info['sku_id'],
                    'appointment_date' => [
                        'in',
            ...
            // 商品库存
            $sum_goods_stock = $goods_stock_model->getSum([
                "goods_id" => $info['goods_id']
                ], "stock");
            $goods_model->save([
                'stock' => $sum_goods_stock
                ], [
                    'goods_id' => $info['goods_id']
                ]);
            // 商品SKU库存
            $sum_sku_stock = $goods_stock_model->getSum([
                "sku_id" => $info['sku_id']
                ], "stock");
            ...
            $goods_model->commit();
            $goods_sku_model->commit();
            $goods_stock_model->commit();
        } catch (\Exception $e) {
            $goods_stock_model->rollback();
            return $retval = array(
                    "code" => 0,
                    "message" => $e->getMessage()
                );

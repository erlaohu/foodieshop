...
class GoodsPreference extends BaseService
{
    ...
    public function getGoodsSkuListPrice($goods_sku_list)
    {
        ...
        if (! empty($goods_sku_list)) {
            ...
            foreach ($goods_sku_list_array as $k => $v) {
                ...
                $appointment_date = $sku_data[2];
                $sku_price = $this->getGoodsSkuPrice($sku_id, $appointment_date);
     ...
     * @param unknown $appointment_date        
     */
    public function getGoodsSkuPrice($sku_id, $appointment_date = "")
    {
        ...
        if (! empty($appointment_date)) {
            ...
            $goods_sku_info['price'] = $goods_stock_info['price'];
            $goods_sku_info['promote_price'] = $goods_stock_info['promote_price'];
        }
        if (! empty($this->uid)) {
            $member_price = $this->getGoodsSkuMemberPrice($sku_id, $this->uid, $appointment_date);
     ...
     * @param unknown $appointment_date            
     */
    public function getGoodsSkuMemberPrice($goods_sku_id, $uid, $appointment_date = "")
    {
        // 查询sku相关信息
        $goods_sku = new NsGoodsSkuModel();

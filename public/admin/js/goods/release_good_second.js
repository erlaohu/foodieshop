...
//		else  $(".h50").hide();
	...
	$(".volume-set-sku-info .btn-common").click(function(){
		...
		if(v.length>0){
			...
			if(is_update){
				...
				}else if(tag == "appointment-day"){
					$(".block-goods-sku>.goods-sku-list .input-common[name='appointment_day_num']").val(v);
				}
	...
	$(".edit-sku-popup-body article>div.sku-display-mode nav ul li").click(function(){
		...
		var curr_sku = $(".edit-sku-popup-body aside>ul li.curr.selected");
		...
		if(show_type != select_show_type){
			$(".edit-sku-popup-body aside>ul li.selected.curr").attr("data-show-type",select_show_type).click();
			//由文字或者颜色展示方式改为图片展示方式的时候，给予标识，后续用于判断是否上传了图片
			if(select_show_type == 3) curr_sku.attr("data-is-update-img",0);
		}
...
function PackageProductInfo() {
	...
	//提前预约天数
	productViewObj.appointment_day = $("#appointment_day").val();
	productViewObj.areaArray = $("#goods_area").val() == null ? 0 : $("#goods_area").val().toString();
...
$(".js-sku-list-popup li.add-sku input").live("keyup",function(event){
	if(event.keyCode == 13){
		...
		var html = "<li data-spec-id="+ predefined_id +" data-spec-value-json='[]' data-spec-name='" + spec_name + "' data-show-type='" + 1 + "'>" + spec_name + "<span>[" + 0 + "/" + 0 + "]</span></li>";
...
$(".edit-sku-popup-body article .add-sku-value-input input").live("keyup",function(event){
	if(event.keyCode == 13){
		...
		var selected_spec = $(".edit-sku-popup-body aside ul li.curr.selected");
			...
			if(res !=-1){
				...
				if(spec_length>0){
					for(var i=0;i<spec_length;i++){
						...
						if($(".js-sku-list-popup li[data-spec-id='" + curr_spec.spec_id + "']").length == 0){
							//第一次加载
							sku_list_html += "<li data-spec-id="+ curr_spec.spec_id +" data-spec-value-json='" + JSON.stringify(curr_spec.values) + "' data-spec-name='" + curr_spec.spec_name + "' data-show-type='" + curr_spec.show_type + "' data-spec-value-length='" + curr_spec.values.length + "'><span>" + curr_spec.spec_name + "</span><span>[" + 0 + "/" + curr_spec.values.length + "]</span></li>";
						}else{
							//显示
							...
							if(is_update){
								...
								$(".js-sku-list-popup li[data-spec-id="+ curr_spec.spec_id +"]")
								...
								$(".js-sku-list-popup li[data-spec-id="+ curr_spec.spec_id +"]").show();
							}
						}
					}
					...
					$(".js-sku-list-popup .add-sku").before(sku_list_html);

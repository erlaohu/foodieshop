...
function createTable(){
	...
	for(var i = 0; i < $sku_array.length; i ++){
		...
		if($temp_Obj[child_id_string] == undefined){
			...
			$temp_Obj[child_id_string]["appointment_day_num"] ="0";
		...
		html += '<tr skuid="' + child_id_string + '">';
			...
			//提前预约天数
			html += '<td>';
				html += '<input type="text" class="input-common middle js-appointment-day" maxlength="9" name="appointment_day_num" value="'+$temp_Obj[child_id_string]["appointment_day_num"]+'" onkeyup="inputKeyUpNumberValue(this);" onafterpaste="inputAfterPasteNumberValue(this);"/>';
				html += '<span class="help-inline" ' + help_inline_style + '>提前预约天数不能为空</span>';
			html += '</td>';
...
function synchroSkuValueData(){
	...
	for(var i = 0; i < $sku_array.length ; i++ ){
		...
		if(sku_str == ""){
			sku_str = sku_id +"¦"+value_array["sku_price"]+"¦"+value_array["market_price"]+"¦"+value_array["cost_price"]+"¦"+value_array["stock_num"]+"¦"+value_array["appointment_day_num"]+"¦"+value_array["code"];
		}else{
			sku_str +="§"+sku_id +"¦"+value_array["sku_price"]+"¦"+value_array["market_price"]+"¦"+value_array["cost_price"]+"¦"+value_array["stock_num"]+"¦"+value_array["appointment_day_num"]+"¦"+value_array["code"];
		}
	}
	...
	if($specObj.length > 0){
		if(sku_list!=null){
			$.each(sku_list,function(c,v){
				...
				$temp_Obj[v.attr_value_items]["appointment_day_num"] =v.appointment_day;

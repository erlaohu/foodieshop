...
(function(root, factory) {
  ...
  $.fn.date = function(options, Ycallback, Ncallback) {
    ...
    var initY = parseInt((nowdate.getFullYear() - opts.beginyear), 10),
      initM = parseInt((nowdate.getMonth() - opts.beginmonth), 10) + 1,
      initD = parseInt((nowdate.getDate() - opts.beginday), 10),
    ...
    function createDateUi() {
      var str = '' +
        ...
        '</div>' +
        '</section>' +
        '<div id="dateFooter" class="date-footer">' +
    ...
    function initIScroll() {
      ...
      yearScroll = new iScroll('yearwrapper', $.extend(true, {}, opts.scrollConfig, {
        onScrollEnd: function() {
          ...
          if (strY == opts.beginyear) {
            $('#monthwrapper').find('ul').html(createMonthUl());
            $('#daywrapper').find('ul').html(createDayUl());
          } else {
            $('#monthwrapper').find('ul').html(createInitMonthUl());
            $('#daywrapper').find('ul').html(createInitDayUl());
          }
          dayScroll.refresh();
        }
      }));
      monthScroll = new iScroll('monthwrapper', $.extend(true, {}, opts.scrollConfig, {
        onScrollEnd: function() {
          ...
          opts.endday = checkdays(strY, strM);
          if (strY == opts.beginyear && strM == opts.beginmonth) {
            $('#daywrapper').find('ul').html(createDayUl());
          } else {
            $('#daywrapper').find('ul').html(createInitDayUl());
          }
